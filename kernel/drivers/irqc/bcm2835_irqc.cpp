...
{
    r = reinterpret_cast<RegisterMap volatile*>(ioremap(m_config.iobase + m_config.offset, sizeof(RegisterMap)));
...
void BCM2835InterruptController::dispatch_irq(InterruptFrame *frame)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        if (basic & mask)
            call_irq(m_basic_irqs[i], "basic", i);
    }

    if (also_check_pending1) {
        for (int i = 0; i < 32; ++i) {
            if (pending1 & (1 << i))
                call_irq(m_pending1_irqs[i], "pending1", i);
        }
    }
    ...
    if (also_check_pending2) {
        for (int i = 0; i < 32; ++i) {
            if (pending2 & (1 << i))
                call_irq(m_pending2_irqs[i], "pending2", i);
        }
    }
}
